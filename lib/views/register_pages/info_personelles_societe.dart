...
class InfoPersonnels extends StatefulWidget {
  const InfoPersonnels({super.key});

  @override
  State<InfoPersonnels> createState() => _InfoPersonnelsState();
}

class _InfoPersonnelsState extends State<InfoPersonnels> {
...
/*     Map<String,dynamic> jsonMap = await user.register(user);
 */
    try {
      ...
      showLoading();
      await api.registerNewUser(formdata);
      hideLoading();

      Navigator.pushReplacement(
          context, MaterialPageRoute(builder: (context) => const PinputPage()));
  ...
  void changeVisibility(State<InfoPersonnels> state, int index) {
    switch (index) {
      case 3:
        state.setState(() {
          thirdPage = !thirdPage;
